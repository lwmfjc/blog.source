...
![站内信 Web 消息推送](attachments/img/ly-20241129104539395.png)
...
![移动端消息推送示例](attachments/img/ly-20241129104539758.png)

Web 端消息推送示例：

![Web 端消息推送示例](attachments/img/ly-20241129104540096.png)

在具体实现之前，咱们再来分析一下前边的需求，其实功能很简单，**只要触发某个事件（主动分享了资源或者后台主动推送消息）**，**Web 页面的通知小红点就会实时的 `+1`** 就可以了。

通常在服务端会有若干张消息推送表，用来记录用户触发不同事件所推送不同类型的消息，前端主动查询（拉）或者被动接收（推）用户所有未读的消息数。

 ![ly-20241129104540434](attachments/img/ly-20241129104540434.png)
...
![ly-20241129104540777](attachments/img/ly-20241129104540777.png)
...
![iframe 流示意图](attachments/img/ly-20241129104541125.png)
...
![iframe 流效果](attachments/img/ly-20241129104541489.png)
...
![SSE 兼容性](attachments/img/ly-20241129104542533.png)
...
是一种在 TCP 连接上进行全双工通信的协议，建立客户端和服务器之间的通信渠道。浏览器和服务器**仅需一次握手**，两者之间就直接可以创建持久性的连接，并进行双向数据传输。
 ...
 ![ly-20241129104542867](attachments/img/ly-20241129104542867.png)
...
页面初始化建立 WebSocket 连接，之后就可以进行双向通信了，效果还不错。
 ...
 ![ly-20241129104543200](attachments/img/ly-20241129104543200.png)
...
![MQTT 协议示例](attachments/img/ly-20241129104543570.png)
