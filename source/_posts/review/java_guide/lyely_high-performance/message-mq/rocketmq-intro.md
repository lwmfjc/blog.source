...
我来举个🌰吧，比如我们有一个购票系统，需求是用户在购买完之后能接收到购买完成的短信。
 ...
 ![img](attachments/img/ly-20241129104620646.jpg)
...
![img](attachments/img/ly-20241129104621097.jpg)
...
![img](attachments/img/ly-20241129104621430.jpg)
...
![img](attachments/img/ly-20241129104621770.jpg)

那么第二步，我们又添加了一个发送邮件，我们就得重新去修改代码，如果我们又加一个需求：用户购买完还需要给他加积分，这个时候我们是不是又得改代码？

![img](attachments/img/ly-20241129104622103.jpg)

如果你觉得还行，那么我这个时候不要发邮件这个服务了呢，我是不是又得改代码，又得重启应用？

 ![ly-20241129104622431](attachments/img/ly-20241129104622431.png)
...
![img](attachments/img/ly-20241129104622772.jpg)
...
我们再次回到一开始我们使用同步调用系统的情况，并且思考一下，如果此时有大量用户请求购票整个系统会变成什么样？
 ...
 ![ly-20241129104623107](attachments/img/ly-20241129104623107.png)
...
![img](attachments/img/ly-20241129104623439.png)

别急，办法总是有的。

## [#](#rocketmq是什么) RocketMQ是什么？

> 原理  ![ly-20241129104623778](attachments/img/ly-20241129104623778.png)
>
...
![img](attachments/img/ly-20241129104624107.jpg)
...
![img](attachments/img/ly-20241129104624443.jpg)
...
![ly-20241129104624814](attachments/img/ly-20241129104624814.png)
...
![img](attachments/img/ly-20241129104625151.jpg)
...
![img](attachments/img/ly-20241129104625487.jpg)

可能你还有一个问题，**为什么一个主题中需要维护多个队列** ？

答案是 **提高并发能力** 。的确，每个主题中只存在一个队列也是可行的。你想一下，如果每个主题中只存在一个队列，这个队列中也维护着每个消费者组的消费位置，这样也可以做到 **发布订阅模式** 。如下图。

![img](attachments/img/ly-20241129104625825.jpg)
...
![ly-20241129104626159](attachments/img/ly-20241129104626159.png)
...
- `Broker`： 主要负责消息的**存储**、**投递**和**查询**以及服务**高可用**保证。说白了就是**消息队列服务器嘛**，生产者生产消息到 `Broker` ，消费者从 `Broker` 拉取消息并消费。
  ...
  **一个 `Topic` 分布在多个 `Broker`上，一个 `Broker` 可以配置多个 `Topic` ，它们是多对多的关系**。
  ...
  ![ly-20241129104626493](attachments/img/ly-20241129104626493.png)
...
- `Consumer`： 消息消费的角色，支持**分布式集群方式部署**。支持以**push推**，**pull拉**两种模式对消息进行消费。同时也支持**集群方式**和**广播方式**的消费，它提供实时消息订阅机制。说白了就是消费者。

听完了上面的解释你可能会觉得，这玩意好简单。不就是这样的么？

![img](attachments/img/ly-20241129104626830.jpg)
...
![img](attachments/img/ly-20241129104627164.jpg)
...
![img](attachments/img/ly-20241129104627504.jpg)
...
![img](attachments/img/ly-20241129104627838.png)
...
![image.png](attachments/img/ly-20241129104628169.png)
...
![img](attachments/img/ly-20241129104628511.jpg)
...
![img](attachments/img/ly-20241129104628843.jpg)
...
![img](attachments/img/ly-20241129104629180.jpg)
