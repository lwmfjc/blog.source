...
- 在一致性非锁定读下，即使读取的记录**已被其他事务加上X锁**，这时记录也是可以被读取的，即读取的**快照数据**。
  ...
  >
  > - 假设有这样一张表  
  >   ![img](attachments/img/ly-20241129104231356.png)
  >
  ...
  >
  > - 时序图如下  
  >   ![ly-20241129104232368](attachments/img/ly-20241129104232368.png)
  >
...
- **事务可见性**示意图（这个图容易理解）：  
  ...
  ![ly-20241129104232712](attachments/img/ly-20241129104232712.png)
...
- **InnoDB**存储引擎中**undo log**分为两种：**insert undo log**和**update undo log**
  ...
  1. **insert undo log**：指在**insert**操作中产生的**undo log**，因为**insert**操作的记录只对事务本身可见，对其他事务不可见，故**该undo log**可以在**事务提交后直接删除**。不需要进行**purge**操作（purge：清洗）
     ...
     **insert**时的数据初始状态：(DB_ROLL_PTR为空)  
     ![ly-20241129104233119](attachments/img/ly-20241129104233119.png)
...
- 数据第一次修改时  
  ![ly-20241129104233467](attachments/img/ly-20241129104233467.png)

- 数据第二次被修改时  
  ![ly-20241129104233809](attachments/img/ly-20241129104233809.png)
...
- 具体的比较算法  
  ![img](attachments/img/ly-20241129104234218.png)
...
举例：  （Tn 表示时间线）  
![img](attachments/img/ly-20241129104234554.png)

## 在RC下ReadView生成情况

**1. 假设时间线来到 T4 ，那么此时数据行 id = 1 的版本链为：**

![img](attachments/img/ly-20241129104234886.png)
...
![img](attachments/img/ly-20241129104235311.png)

**2. 时间线来到 T6 ，数据的版本链为：**

![img](attachments/img/ly-20241129104235642.png)
...
**3. 时间线来到 T9 ，数据的版本链为：**  
![img](attachments/img/ly-20241129104236037.png)

![img](attachments/img/ly-20241129104236393.png)
...
![img](attachments/img/ly-20241129104236730.png)

**1. 在 T4 情况下的版本链为：**

![img](attachments/img/ly-20241129104237096.png)
...
![img](attachments/img/ly-20241129104237446.png)
...
![img](attachments/img/ly-20241129104237777.png)
